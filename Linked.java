...
import javafx.application.Application; import java.util.Scanner; import java.io.*; import java.util.LinkedList;
...
public static void main(String[] args) {
  ...
  //Populate the list with sample station names...
  ...
  //The reason we have this output is to give the user an idea of what the output will looks like
    myLinkList.listDelete(new trainUserTyped("Hynes"));

    System.out.println("After deleting Hynes, the list is:");
//Print updated list myLinkList.printList(); System.out.println("----------------------");

    //Now we get out of the simulation and begin using real input
  
  //Make sure you can pick up that input with a scanner
  ...
  //Since we're expecting an integer answer, look for that.
    int answer=scan.nextInt();  
    
  
  //Each answer corresponds to a menu item
...
class trainUserTyped { String train;
...
//Dummy node public NODE() { info = new trainUserTyped("\0"); }
...
LinkList() {  //Think of this as a 1950's phone tree: each person will tell you who to call next.
    //initialize Node
    Node = new NODE();
...
NODE currentNode = Node; //This is set to Node because he comes first NODE previousNode = null; //null because you haven't done anything yet
